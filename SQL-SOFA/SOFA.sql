DROP MATERIALIZED VIEW IF EXISTS mimiciii_sofa.SOFAperhour CASCADE;
CREATE materialized VIEW mimiciii_sofa.SOFAperhour AS
...
with scorecomp as (
  ...
  , cv.MinBP
	...
	, rl.Creatinine
 	from admissions ha
    left join (	
	    select hadm_id, hlos from mimiciii_sofa.SOFA_PaO2FiO2 union
	    select hadm_id, hlos from mimiciii_sofa.coag_labsperhour union
	    select hadm_id, hlos from mimiciii_sofa.liv_labsperhour union
	    select hadm_id, hlos from mimiciii_sofa.SOFA_cardio union
	    select hadm_id, hlos from mimiciii_sofa.cardio_vitalsperhour union
	    select hadm_id, hlos from mimiciii_sofa.SOFA_gcsperhour union
	    select hadm_id, hlos from mimiciii_sofa.SOFA_runninguo24h union
	    select hadm_id, hlos from mimiciii_sofa.ren_labsperhour
    ) as u	
        on ha.hadm_id = u.hadm_id
      left join mimiciii_sofa.SOFA_PaO2FiO2 pf
        on u.hadm_id = pf.hadm_id
        and u.hlos = pf.hlos
      left join mimiciii_sofa.coag_labsperhour cl
        on u.hadm_id = cl.hadm_id
        and u.hlos = cl.hlos
      left join mimiciii_sofa.liv_labsperhour ll
        on u.hadm_id = ll.hadm_id
        and u.hlos = ll.hlos
      left join mimiciii_sofa.SOFA_cardio c
        on u.hadm_id = c.hadm_id
        and u.hlos = c.hlos
      left join mimiciii_sofa.cardio_vitalsperhour cv
        on u.hadm_id = cv.hadm_id
        and u.hlos = cv.hlos
      left join mimiciii_sofa.SOFA_gcsperhour gcs
        on u.hadm_id = gcs.hadm_id
        and u.hlos = gcs.hlos
      left join mimiciii_sofa.SOFA_runninguo24h uo
        on u.hadm_id = uo.hadm_id
        and u.hlos = uo.hlos
      left join mimiciii_sofa.ren_labsperhour rl
...
, SOFA_per_hour as (
  ...
  from mimiciii_sofa.hadms_hours hah
  left join SOFA
